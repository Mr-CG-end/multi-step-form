        ...
        <div class="content">
          ...
          <div class="forms">
              ...
              <div v-if="commonsStore.nowTab === '1'">
                <div class="form form-name">
                  <div class="labels">
                    ...
                    <label for="name" v-if="validation.nameVal === false" class="alert">此字段为必填项</label>
                  </div>
                  <input v-model="commonsStore.personalInfo.name" type="text" id="name" placeholder="例如：张三" :class="[{'error': validation.nameVal === false}]" required />
                </div>
                
                <div class="form form-email">
                  <div class="labels">
                    <label for="email" class="label-name">电子邮件地址</label>
                    <label for="email" v-if="validation.emailVal === false" class="alert">此字段为必填项</label>
                  </div>
                  <input v-model="commonsStore.personalInfo.email" type="text" id="email" placeholder="例如：zhangsan@example.com" :class="[{'error': validation.emailVal === false}]" required />
                </div>
                
                <div class="form form-phone">
                  <div class="labels">
                    <label for="phone" class="label-name">电话号码</label>
                    <label for="phone" v-if="validation.phoneVal === false" class="alert">此字段为必填项</label>
                  </div>
                  <input v-model="commonsStore.personalInfo.phone" type="text" id="phone" placeholder="例如：+86 138 0000 0000" :class="[{'error': validation.phoneVal === false}]" required/>
                </div>
              </div>
                ...
                <div class="costs">
                  ...
                  <div :class="['plan-wrap', {'plus' : commonsStore.addons.length !== 0}]">
                    <div class="plan">
                      ...
                      <div @click="()=>commonsStore.nowTab = '2'" class="change-plan">更改</div>
                    </div>
            ...
            <div :class="['btns', {'none': commonsStore.nowTab === '5'}]" v-if="commonsStore.nowTab !== '5'">
              <button class="lft-btn" @click="goBack" v-if="commonsStore.nowTab !== '1'">返回</button>
              <button class="rgt-btn" v-if="commonsStore.nowTab !== '4'" @click="onSubmit">下一步</button>
              <button class="rgt-btn confirm" v-else-if="commonsStore.nowTab === '4'" @click="onSubmit">确认</button>
            </div>
        </div>
